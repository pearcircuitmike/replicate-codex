  ...
  const fetchSummary = async () => {
    try {
      const response = await fetch(
        `/api/weekly-summaries-models?offset=${offset}`
      );
      const data = await response.json();
      setSummary(data.data);
      setTotalCount(data.totalCount);
    } catch (error) {
      console.error("Error fetching weekly model summary:", error);
    }
  };
  ...
  return (
    <VStack spacing={4} align="stretch" mx={[2, 10, 10]}>
      <Heading as="h1" size="xl">
        AI Models Weekly Summary
      </Heading>
      <Text>
        This page provides a weekly summary of the most significant AI model
        releases and updates. Each summary highlights key capabilities,
        architectures, and potential impacts of new artificial intelligence
        models released in the last seven days. Stay informed about the rapidly
...
export default WeeklyModelSummary;
