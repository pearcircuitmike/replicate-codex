...
export default async function handler(req, res) {
  ...
  try {
    // Check the current value of signup_source
    ...
    // If signup_source is already set, treat as a conflict (409)
    if (profile.signup_source) {
      return res.status(409).json({ message: "Signup source already set" });
    }
